(() => {
  ...
  const REMOVE_TEXTS = [
    "Merkez doÄrulama ve oturum yÃ¶netimi kurumsal e-posta hattÄ± Ã¼zerinden ilerler.",
    "Merkez do\u011frulama ve oturum yonetimi kurumsal e-posta hatti uzerinden ilerler.",
    "Merkez kanalÄ±",
    "Merkez kanal\u0131",
    "DoÄrudan merkez ile irtibat, oturum ve doÄrulama akÄ±ÅlarÄ±nÄ± tek panelde takip etmek iÃ§in merkez dÃ¼Ämesini kullan.",
    "Dogrudan merkez ile irtibat, oturum ve dogrulama akislarini tek panelde takip etmek icin merkez dugmesini kullan.",
    "Operasyon notu",
    "Operasyon notu, hÄ±zlÄ± analiz, geÃ§miÅ ve rapor Ã§Ä±ktÄ±larÄ± daÄÄ±nÄ±k ekran hissi oluÅturmadan aynÄ± akÄ±Åta kalÄ±r.",
    "AktÃ¶r, zaman, lokasyon, tetikleyici, muhtemel hedef ve belirsizlik seviyesini aynÄ± olay akÄ±ÅÄ± iÃ§inde vermek sonuÃ§ kalitesini ciddi biÃ§imde artÄ±rÄ±r.",
  ...
  const stripKnownCards = () => {
    qa(".sidebar-card, .hero-card").forEach((card) => {
      ...
      if (
        clean.includes(cleanForMatch("Merkez kanalÄ±")) ||
        clean.includes(cleanForMatch("DoÄrudan merkez ile irtibat, oturum ve doÄrulama akÄ±ÅlarÄ±nÄ± tek panelde takip etmek iÃ§in merkez dÃ¼Ämesini kullan.")) ||
        clean.includes(cleanForMatch("Operasyon notu")) ||
        clean.includes(cleanForMatch("Operasyon notu, hÄ±zlÄ± analiz, geÃ§miÅ ve rapor Ã§Ä±ktÄ±larÄ± daÄÄ±nÄ±k ekran hissi oluÅturmadan aynÄ± akÄ±Åta kalÄ±r."))
      ) {
        card.remove();
      }
    });
    ...
    qa(".auth-footer .company-line").forEach((line) => {
      ...
      if (clean.includes(cleanForMatch("Merkez doÄrulama ve oturum yÃ¶netimi kurumsal e-posta hattÄ± Ã¼zerinden ilerler."))) {
        line.remove();
      }
    });
  };
